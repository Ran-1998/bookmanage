...
public class AddToDBBook extends MybatisConfig{
	...
	public void addOne() throws IOException {
		...
		for (String stringOne : one) {
			category.setId(null).setIsParent(true).setName(stringOne).setStatus(1).setParentId(0l).setCreated(new Date())
					.setUpdated(new Date());
	...
	public void addTwo() throws IOException {
		...
		for (Entry<String, List<String>> entry : set) {
			...
			for (String s : value) {
				category.setId(null).setIsParent(true).setName(s).setStatus(1).setParentId(selectParentId.longValue())
	...
	public void addThree() throws IOException {
		...
		for (Entry<String, List<String>> entry : set) {
			...
			for (String s : value) {
				...
				category.setId(null).setIsParent(false).setName(s).setStatus(1).setParentId(selectParentId.longValue())
						.setCreated(new Date()).setUpdated(new Date());
				if (categoryMapper.selectParentId(s) == null) {
					categoryMapper.addCategory(category);
				}
	...
	public void addBook() throws IOException, ParseException {
		 ...
		 */
			Elements elements = CategoryUtil.getElement(url);
					...
					for (Element element3 : select3) {
						...
						if (bookurls.contains("category.dangdang")) {
							...
							for (Element element4 : bookElement) {
								...
								book.setId(id).setIsbn(isbn).setBookname(name).setAuthor(author).setPress(press).setCid(cid).setStatus(1l)
								.setBriefintroduction(briefintroduction).setPublicationdate(d).setImage(image).setCreated(new Date()).setUpdated(new Date());
